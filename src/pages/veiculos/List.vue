<script setup lang="ts">
    ...
    const handleEdit = (row: any) => {
        router.push({ name: 'veiculos.editar', params: { id: row.id } })
    }

    const handleDelete = async (row: any) => {
        loading.value = true

        let response = await api.delete(`/veiculos/${row.id}`)
...
<template>
    ...
    <el-row align="middle">
        <el-col :span="12">
            <h1>Veículos - Listar</h1>
        </el-col>
        <el-col :span="12" class="text-right">
            <el-button color="#1e3454" @click="goTo('veiculos.cadastrar')">Cadastrar</el-button>
        </el-col>
    </el-row>
